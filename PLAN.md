...
4b. **ROMAN — cerrar la verificación en vivo** (trazado formal iniciado 6 sep 2026, ver [`plan_relevamiento_alta_roman.md`](plan_relevamiento_alta_roman.md)). Capas 4/6/7 cerradas *a nivel config* en `OPENWLPROD01`: secciones `[csm]*` reales en `formsweb.cfg` (`pageTitle=CSM PRODUCCION`, `userid=@PRODCSM`), `PRODCSM`/`QACSM` → `OPENDBPROD03` (`10.77.7.30:1525`), sin NFS, WL `12.2.1.4.0` (cierra la fila de Discrepancias — sube de Tier 3 a acá porque el trazado ya está en curso). **Pero cero tráfico Forms de ROMAN en 5 días de `access.log` de `WLS_FORMS`** y 0 conexiones a `.30` en el `netstat` — ROMAN está configurado como producción pero puede estar dormido. Falta: (a) access log del NPM de `DOCKER-DEB` para `romanprod.condor.solutions` (proxy host id 93; el NPM corre en contenedor); (b) `v$session` en `OPENDBPROD03` (bloqueado por credencial de DB, misma que falta para `10.77.7.15`/`.22` — `QUESTIONS.md`). Cabo aparte: clasificar `OPENDBDES011` (`10.77.7.151`, "Roman test" en vCenter), descartado de la ruta productiva pero sin trazar qué corre ahí.
