...
**CEFAS — destino de migración de la DB identificado (18 ago 2026).** `matrix_detail.ip_db_destino` de CEFAS (`10.77.7.11`) coincide exacto con la VM `OPENDBPROD001`, ya presente en el inventario como base de datos de **Heinlein**. Sería una instancia Oracle compartida entre dos clientes, mismo patrón que otras DBs multi-cliente del proyecto (ej. `DBClientes.190` para ABB/DCVIAJES). Falta confirmar el SID (`prodcefas` según la matriz) y si ya es la productiva — ver `plan_relevamiento_alta_cliente.md` capa 5.

**Credencial de pfSense — confirmada, no bloqueada (18 ago 2026).** Acceso real logrado al dashboard de `FWOPEN` (`192.1.1.11`) durante el trazado punta a punta de CEFAS. A diferencia de SSH/Portainer/NPM (siguen sin credencial), pfSense ya no es un bloqueador — ver `plan_relevamiento_alta_cliente.md`.
