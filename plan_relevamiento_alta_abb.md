...
SSH directo (acepta `soportesmart`). `sudo docker exec ssl-app-1 sh -c 'find /data/logs -name "proxy_host-*.log" -exec grep -l "abb.condorwork.com.ar" {} \;'`, después `tail`/`grep "POST /forms/lservlet"` en el archivo que salga. **Gotcha ya conocido:** el número de archivo `proxy_host-<N>.log` **no** coincide con el `id` de la tabla — ubicarlo por contenido. Ver `plan_relevamiento_alta_jobs.md` punto 2.
