...
| 6 | Base de datos | ⬜ **Único paso en vivo que queda. Host ya accesible.** `CLIENTES-DB` (`192.1.1.32`) se accedió el 25 ago 2026 (trazado CEFAS); `ps -ef \| grep pmon` **ya listó `ora_pmon_BOCA`** ahí — es la DB candidata (matriz: `CLIENTES-DB` / puerto 1525 / SID `BOCA`/`BOCAPDB`). **Toca prod de BOCA → correr lunes a sábado, no domingo.** | Reconectar a `192.1.1.32`: `export ORACLE_SID=BOCA; sqlplus / as sysdba` → `SELECT name, open_mode FROM v$database;` cierra `BOCA` vs `BOCAPDB`. `SELECT value FROM nls_database_parameters WHERE parameter='NLS_CHARACTERSET';` confirma `WE8MSWIN1252`. `SELECT username,machine,program,count(*) FROM v$session WHERE username IS NOT NULL GROUP BY username,machine,program;` → esperar `machine=192.1.2.54` (confirma que `WLS_FORMS` pega acá). |
