...
| 5 | Base de datos | **Confirmado qué DB usa hoy (25 ago 2026).** `netstat -tn` en `WebLogic.191` (sin sudo, alcanza) muestra 2 conexiones `ESTABLISHED` en vivo a `192.1.1.32:1525` — `CLIENTES-DB`, la actual. **Cero tráfico hacia `10.77.7.11` (`OPENDBPROD001`, destino de migración)** — la migración todavía no cortó tráfico productivo, sigue siendo un destino futuro, no el vigente. SID en disputa (`CEFAS` vs `CEFASPDB`) sigue sin confirmar. **Bloqueado por credencial (25 ago 2026):** ni SSH ni consola VMware/Veeam a `CLIENTES-DB` (`192.1.1.32`) aceptan acceso — probado también el usuario/contraseña que sí funcionó en `WebLogic.191`/`VM-DOCKER-Clientes`, sin éxito. Mismo patrón de bloqueo que SSH/Portainer/NPM al inicio del proyecto. | Sin credencial propia, no hay forma de seguir por TeamViewer. **Anotar para la reunión con el equipo saliente:** pedir acceso (SSH o consola) a `CLIENTES-DB`. Una vez adentro: `sqlplus / as sysdba` → `SELECT name FROM v$database;`. |
