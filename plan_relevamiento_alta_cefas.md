...
| 4a | App — motor clásico | **Resuelto por qué daba connection refused.** De las 45 reglas NAT de `FWOPEN`, solo 3 apuntan a `192.1.1.191` (`8089`→Mafisa, `8088`→DVAL, `8191`→UIA), **las tres al puerto `80`** — ninguna al `7001`. La consola de admin nunca estuvo expuesta por NAT; el refused era esperable, no una falla del servidor. `WebLogic.191` sirve `cefas.condorwork.com.ar` normalmente por el `80` (vía el NPM de `VM-DOCKER-Clientes`, capa 4b/2). | Para la consola de admin hace falta entrar desde dentro de la red (TeamViewer a un host interno), no desde afuera — el puerto `7001` no tiene ruta externa. Una vez adentro: Data Sources → URL JDBC, contra cuál DB apunta hoy. |
...
| 5 | Base de datos | `CLIENTES-DB` (`192.1.1.32`) es la actual, SID en disputa (`CEFAS` vs `CEFASPDB`). **Confirmado con acceso externo real:** la tabla de NAT de `FWOPEN` tiene una regla dedicada, "mitiga Cefas DB 1525" — puerto WAN `11517` → `192.1.1.32:1525` (puerto Oracle no estándar, no el `1521` por defecto). Destino de migración: `OPENDBPROD001` (`10.77.7.11`), ya compartida con Heinlein, sin regla NAT propia todavía en la lista relevada. | `sqlplus / as sysdba` → `SELECT name FROM v$database;` en `OPENDBPROD001` (destino) y, si hace falta comparar, en `CLIENTES-DB` vía el puerto `1525` ya confirmado. Cruzar contra el datasource del paso 4a. |
