...
- **No aparece en ningún dump de NPM transcripto** (`DOCKER-DEB-NginxProxyManager/proxy_hosts.csv`, `dockerdeb_proxy_host_2026-09-01.tsv`) — consistente con "no usa dominio", pero esos dumps no cubren los otros dos NPM que `plan_relevamiento_alta_roman.md` deja pendientes (`OPENDOCKER04`, `VM-DOCKER-Clientes (1)`).
