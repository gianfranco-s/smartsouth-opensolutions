...
|---|---|---|---|
| 1 | Dominio de entrada | 🟡 **Identificado, sin clasificar por tráfico.** Ruta actual: `romanprod.condor.solutions` (proxy host id **93**) / `romanqa.condor.solutions` (id **94**) → `10.77.7.201:9001` (`OPENWLPROD01`), ambos habilitados en `DOCKER-DEB`. ORDS: `ords-roman2.open.com.ar` (id 85) → `10.77.7.12:8040` (habilitado, `.12` sin VM). Legado (`roman.condorwork.com.ar`, `roman.condorenterprise.com.ar` → `172.18.5.40`) y `ords-romans.open.com.ar` → `200.55.243.116:2235`: deshabilitados. **Backend confirmado** (capa 4), pero el `WLS_FORMS` no vio ni un request `config=csm` en 5 días. | **PRÓXIMO PASO INMEDIATO.** En `DOCKER-DEB` (`192.1.1.37`): el NPM corre en contenedor (no hay `/data/logs/` suelto) — `sudo docker ps` → ubicar el contenedor NPM → `sudo docker exec <c> ls -la /data/logs` → `grep` el `proxy-host-93_access.log` por requests `POST /forms/lservlet 200` recientes. Si está vacío → ROMAN está dormido pese a los labels "PRODUCCION". *Completeness*: `OPENDOCKER04` / `VM-DOCKER-Clientes (1)` sin otra ruta ROMAN. |
