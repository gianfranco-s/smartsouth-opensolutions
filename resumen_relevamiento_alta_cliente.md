...
| Base de datos | ✅ Resuelto | ✅ Resuelto | ✅ Resuelto (ambas rutas — `eby-prod` real, `yacyreta` descartada) | ✅ Resuelto — CDB `BOCA` / PDB `BOCAPDB`, schema `CONDOR` |
| Almacenamiento / object store | ✅ Resuelto | *(no aplica a este cliente)* | ✅ Resuelto — no aplica | ✅ Resuelto — no aplica |

**CEFAS, JOBS, EBY y BOCA: trazados completos, 7/7.** **EBY cerró su último cabo el 12 sep 2026:** `sqlplus` directo a `Database .90`/`CDRADM` (`root@192.1.1.90`) confirmó que esa DB compartida de la ruta paralela `yacyreta` no es de EBY — de los 8 inquilinos de `WebLogic.191`, solo `SIGO` tiene schema propio ahí (2.6 GB, uso activo el mismo día). Cierra por descarte, no por hallazgo positivo: la única DB real de EBY sigue siendo `OPENDBPROD005` (ruta `eby-prod`). Bono: **SIGO** (uno de los 4 clientes de `.191` sin VM mapeada) gana su primera DB confirmada en vivo del proyecto. **BOCA cerró su capa 6 el 12 sep 2026:** `root@192.1.1.32`, `ORACLE_SID=BOCA` reveló que es una **CDB** multitenant casi vacía — los datos reales viven en la PDB **`BOCAPDB`**, schema `CONDOR` (~22.5 GB), charset `WE8MSWIN1252` confirmado, con último DML **el mismo día de la consulta** (uso productivo activo, la mejor evidencia de capa 6 del proyecto). Resuelve la disputa "BOCA vs BOCAPDB": las dos fuentes tenían razón, a distinto nivel (CDB vs PDB) — mismo patrón que Rex/GIAR. El rédito planeado "una pantalla de consola cierra ABB de paso" **no salió** en `192.1.2.54` (sin consola ni sudo) — pero **ABB se cerró después por `sqlplus` directo con `root@192.1.1.31`: 7/7 capas**. DB productiva = `192.1.1.31` (instancia `ABB`, schema `CONDOR`); `192.1.1.190` descartada. Hallazgo colateral: **ABB apagado de hecho desde el 1‑jul‑2026** (cero sesiones/DML desde). Bonos: capa 6 en vivo para **ESYOP** (en uso, co‑inquilina de `.31`) y **DCVIAJES** (`.190`, parcial). **ROMAN: 6 de 7 capas resueltas** (sesiones 6 y 12/13 sep) — dominio, NPM, motor clásico, DB (config) y almacenamiento cerrados en `OPENWLPROD01`/`OPENDBPROD03` (secciones `[csm]*` reales, `PRODCSM` → `OPENDBPROD03`, WL `12.2.1.4.0`); faltan firewall/NAT y capa Docker (bajo costo). **Confirmado sin un solo usuario real** — dos logs independientes (`WLS_FORMS` y el NPM de `DOCKER-DEB`, 0 `lservlet` en 5.097 requests) coinciden en cero tráfico. Credencial `root@10.77.7.30` probada y rechazada (12 sep) — capa 6 sigue sin `v$session`. Abierta en `QUESTIONS.md` la pregunta de negocio: ¿mismo caso que ABB (apagado de hecho)?
...
- **Base de datos** — cerrada para la ruta `eby-prod`: `tnsnames.ora` real de `OPENWLPROD01` confirma alias `EBYPROD`/`EBYQA`/`EBYAUDIT` → `OPENDBPROD005` (`10.77.7.15`), coincide letra por letra con la matriz. **[12 sep 2026] La ruta `yacyreta` queda cerrada por descarte:** `sqlplus` directo a `Database .90`/`CDRADM` (`root@192.1.1.90`) confirmó que, de los 8 inquilinos de `WebLogic.191`, solo `SIGO` tiene schema propio ahí (2.6 GB, uso activo el mismo día) — ni EBY ni ningún otro de los 7 restantes. `Database .90` no es la DB de EBY.
...
- **Rédito colateral** — el mismo trazado (vía re-dump de NPM + `tnsnames.ora`) destrabó infraestructura nueva para **ROMAN** (WL y DB candidatos, primera vez con datos reales), **GIAR** (segunda DB candidata), **Rex Argentina** (primera DB jamás mapeada) y **Heinlein** (ambiente de test confirmado); y el cierre final de `Database .90` (12 sep) resolvió de paso el blind spot de **SIGO** (uno de los 4 clientes de `.191` sin VM mapeada, ahora con DB confirmada en vivo).

**Falta:** nada — trazado de EBY completo (7/7).
...
**Descubierto (sesión 12/13 sep 2026):**
